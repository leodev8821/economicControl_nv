...
import type { WeekSearchData } from '../models/week.model';
import { WeekActions, WeekCreationAttributes, WeekAttributes, WeekModel } from '../models/week.model';
import { WeekCreationSchema, WeekCreationRequest, WeekUpdateSchema, WeekUpdateRequest } from '../schemas/week.schema';
...
export const weeksController = {
    ...
    allWeeks: async (_req: Request, res: Response) => {
        try {
            ...
            return ControllerErrorHandler(res, error, 'Error al obtener las semanas.');
        }
    },
    ...
    oneWeek: async (req: Request, res: Response) => {
        try {
            ...
            if (!week) {
                return res.status(404).json({ message: 'No se encontró la semana con los parámetros proporcionados.' });
            }
    ...
    createWeek: async (req: Request, res: Response) => {
        try {
            ...
            const newWeek = await WeekActions.create(weekData as WeekCreationAttributes);
    ...
    generateWeeks: async (req: Request, res: Response) => {
        try {
            ...
            return ControllerErrorHandler(res, error, 'Error al generar las semanas del año.' );
        }
    },
    ...
    getWeeksByYear: async (req: Request, res: Response) => {
        try {
            ...
            return ControllerErrorHandler(res, error, 'Error al obtener las semanas por año.' );
        }
    },
    ...
    updateWeek: async (req: Request, res: Response) => {
        try {
            ...
            if (!weekId) {
                return res.status(400).json({ ok: false, message: 'ID de semana inválido' });
            }
            ...
            const updatedWeek = await WeekActions.update(weekId, updateData as Partial<WeekCreationAttributes>);

            if (!updatedWeek) {
                return res.status(404).json({ ok: false, message: 'Semana no encontrada para actualizar.' });
            }
            ...
            return ControllerErrorHandler(res, error, 'Error al actualizar la semana.' );
        }
    },
    ...
    deleteWeek: async (req: Request, res: Response) => {
        try {
            ...
            if (!deleted) {
                return res.status(404).json({ ok: false, message: 'No se encontró la semana para eliminar.' });
            }
