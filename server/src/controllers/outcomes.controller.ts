...
import type { OutcomeSearchData } from '../models/outcome.model';
import { OutcomeActions, OutcomeCreationAttributes, OutcomeAttributes } from '../models/outcome.model';
import { OutcomeCreationSchema, OutcomeCreationRequest, OutcomeUpdateSchema, OutcomeUpdateRequest } from '../schemas/outcome.schema';
...
export const outcomesController = {
    ...
    allOutcomes: async (_req: Request, res: Response) => {
        try {
            ...
            return ControllerErrorHandler(res, error, 'Error al obtener las egresos.');
        }
    },
    ...
    oneOutcome: async (req: Request, res: Response) => {
        try {
            ...
            if (!outcome) {
                return res.status(404).json({ message: 'No se encontró la egreso con los parámetros proporcionados.' });
            }
    ...
    outcomesByCash: async (req: Request, res: Response) => {
        try {
            ...
            return ControllerErrorHandler(res, error, 'Error al obtener los egresos por caja.' );
        }
    },
    ...
    createOutcome: async (req: Request, res: Response) => {
        try {
            ...
            const newOutcome = await OutcomeActions.create(outcomeData as OutcomeCreationAttributes);
    ...
    updateOutcome: async (req: Request, res: Response) => {
        try {
            ...
            if (!outcomeId) {
                return res.status(400).json({ ok: false, message: 'ID de egreso inválido' });
            }
            ...
            const updatedOutcome = await OutcomeActions.update(outcomeId, updateData as Partial<OutcomeCreationAttributes>);

            if (!updatedOutcome) {
                return res.status(404).json({ ok: false, message: 'Egreso no encontrada para actualizar.' });
            }
            ...
            return ControllerErrorHandler(res, error, 'Error al actualizar la egreso.' );
        }
    },
    ...
    deleteOutcome: async (req: Request, res: Response) => {
        try {
            ...
            if (!deleted) {
                return res.status(404).json({ ok: false, message: 'No se encontró la egreso para eliminar.' });
            }
