...
export const consolidationController = {
  ...
  allConsolidations: async (_req: Request, res: Response) => {
    try {
      const consolidations: ConsolidationAttributes[] = await ConsolidationActions.getAll();
      ...
      return ControllerErrorHandler(res, error, "Error al obtener las consolidaciones.");
    }
  },
  ...
  oneConsolidation: async (req: Request, res: Response) => {
    try {
      ...
      const consolidationObtained = await ConsolidationActions.getOne(searchCriteria);

      if (!consolidationObtained) {
        return res.status(404).json({
          message: "No se encontró la consolidación con los parámetros proporcionados.",
        });
      }
      ...
      return ControllerErrorHandler(res, error, "Error al obtener la consolidación.");
    }
  },
  ...
  createConsolidation: async (req: Request, res: Response) => {
    try {
      ...
      const consolidationData: ConsolidationCreationAttributes = validationResult.data;

      const newConsolidation = await ConsolidationActions.create(consolidationData);
      ...
      return ControllerErrorHandler(res, error, "Error al crear la consolidación.");
    }
  },
  ...
  updateConsolidation: async (req: Request, res: Response) => {
    try {
      ...
      const updatedConsolidation = await ConsolidationActions.update(
        consolidationId,
        updateData as Partial<ConsolidationCreationAttributes>,
      );

      if (!updatedConsolidation) {
        return res
          .status(404)
          .json({ ok: false, message: "Consolidación no encontrada para actualizar." });
      }
      ...
      return ControllerErrorHandler(res, error, "Error al actualizar la consolidación.");
    }
  },

  deleteConsolidation: async (req: Request, res: Response) => {
    try {
      const consolidationId: number = parseInt((req.params.id as string) || "0", 10);
      ...
      if (!deleted) {
        return res
          .status(404)
          .json({ ok: false, message: "No se encontró la consolidación para eliminar." });
      }
      ...
      return ControllerErrorHandler(res, error, "Error al eliminar la consolidación.");
    }
  },
};
