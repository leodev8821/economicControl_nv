...
import { UserActions, UserAttributes, UserCreationAttributes, UserRole } from '../models/user.model';
import { UserCreationSchema, UserCreationRequest, UserUpdateSchema, UserUpdateRequest } from '../schemas/user.schema';
import type { UserSearchData } from '../models/user.model';
...
export const usersController = {
    ...
    loginUser: async (username: string, password: string): Promise<LoginResult> => {
    ...
    allUsers: async (_req: Request, res: Response) => {
        try {
            ...
            return ControllerErrorHandler(res, error, 'Error al obtener las usuarios.');
        }
    },
    ...
    oneUser: async (req: Request, res: Response) => {
        try {
            ...
            if (!user) {
                return res.status(404).json({ message: 'No se encontró la usuario con los parámetros proporcionados.' });
            }
    ...
    getOneVisible: async (identifier: string | number): Promise<UserAttributes | null> => {
    ...
    createUser: async (req: Request, res: Response) => {
        try {
            ...
            const existingUser = await UserActions.getOne({ username: userData.username });
            ...
            const newUser = await UserActions.create(userData as UserCreationAttributes);
    ...
    updateUser: async (req: Request, res: Response) => {
        try {
            ...
            const existingUser = await UserActions.getOne({ username: updateData.username });
            ...
            const updatedUser = await UserActions.update(userId, updateData as Partial<UserCreationAttributes>);

            if (!updatedUser) {
                return res.status(404).json({ ok: false, message: 'Usuario no encontrada para actualizar.' });
            }
            ...
            return ControllerErrorHandler(res, error, 'Error al actualizar la usuario.' );
        }
    },
    ...
    deleteUser: async (req: Request, res: Response) => {
        try {
            ...
            if (!deleted) {
                return res.status(404).json({ ok: false, message: 'No se encontró la usuario para eliminar.' });
            }
            ...
            return ControllerErrorHandler(res, error, 'Error al eliminar la usuario.' );
        }
    }
};