...
export default function IncomeForm() {
    ...
    const [form, fields] = useForm({
        ...
        onValidate({ formData }) {
            return parseWithZod(formData, { schema: SharedIncomeSchemas.CreateIncomeSchema });
        },
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const submission = parseWithZod(formData, { schema: SharedIncomeSchemas.CreateIncomeSchema });
    ...
    return (
        <form method="post" id={form.id} onSubmit={handleSubmit} className="income-form">
            ...
            <div>
                <label htmlFor={fields.cash_id.id}>Caja</label>
                <select 
                    id={fields.cash_id.id}
                    name={fields.cash_id.name}
                    // Deshabilitado si el formulario está ocupado o si las cajas no han cargado
                ...
                >
                    ...
                    <option value="">
                        ...
                        {!isLoadingCashes && !isErrorCashes && availableCashes?.length > 0 && '-- No aplicar a una caja --'}
                    </option>
                    
                    {/* 🚨 Renderizar opciones solo si los datos están listos */}
                    {!isLoadingCashes && !isErrorCashes && availableCashes && availableCashes.map(c => (
                        // La interfaz Cash tiene 'id', 'name'
                        <option key={c.id} value={c.id}>{c.name}</option> 
                    ))}
                </select>
                {fields.cash_id.errors && <div style={{ color: 'red' }}>{fields.cash_id.errors}</div>}
                
                {/* Opcional: Mostrar error de carga de forma separada si el select está disabled */}
                {isErrorCashes && <div style={{ color: 'red', marginTop: '5px' }}>Hubo un error al cargar la lista de cajas.</div>}
            </div>
            ...
            <div>
                ...
                >
                    ...
                    {SharedIncomeSchemas.INCOME_SOURCES.map((source) => (
