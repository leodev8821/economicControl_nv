...
export default function PersonForm({ initialValues, onSubmit, onCancel, isLoading = false, isUpdateMode = false }: PersonFormProps) {
    const theme = useTheme();

    // 1. Inicialización de Conform
    const [form, fields] = useForm({
        onValidate({ formData }) {
            return parseWithZod(formData, { schema: SharedPersonSchemas.PersonCreationSchema });
        },
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const submission = parseWithZod(formData, { schema: SharedPersonSchemas.PersonCreationSchema });
    ...
    return (
        <form method="post" id={form.id} onSubmit={handleSubmit} className="person-form">
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                    <FormControl fullWidth error={!!fields.first_name.errors}>
                        ...
                        <OutlinedInput
                            ...
                            disabled={isLoading}
                        />
                        {fields.first_name.errors && <FormHelperText>{fields.first_name.errors}</FormHelperText>}
                    </FormControl>
                </Grid>
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                    <FormControl fullWidth error={!!fields.last_name.errors}>
                        ...
                        <OutlinedInput
                            ...
                            disabled={isLoading}
                        />
                        {fields.last_name.errors && <FormHelperText>{fields.last_name.errors}</FormHelperText>}
                    </FormControl>
                </Grid>
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                    <FormControl fullWidth error={!!fields.dni.errors}>
                        ...
                        <OutlinedInput
                            ...
                            disabled={isLoading}
                        />
                        {fields.dni.errors && <FormHelperText>{fields.dni.errors}</FormHelperText>}
                    </FormControl>
                </Grid>
                ...
                <Grid size={12}>
                    <Stack direction="row" spacing={2} justifyContent="flex-end">
                        ...
                        <Button 
                            ...
                            sx={{ minWidth: 150 }}
                        >
                            {isLoading ? 'Guardando...' : (isUpdateMode ? 'Actualizar Persona' : 'Guardar Persona')}
                        </Button>
                    </Stack>
                </Grid>
            </Grid>
